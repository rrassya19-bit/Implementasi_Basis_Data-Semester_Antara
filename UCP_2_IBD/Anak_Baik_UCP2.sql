...
-- Nama Function: UPPER()  = Mengubah seluruh huruf kecil pada suatu teks menjadi huruf kapital (huruf besar) semua
--              : CONCAT() = Menggabungkan beberapa kolom teks atau string menjadi satu kesatuan teks yang menyambung
...
-- Nama Function: GETDATE()  = Mengambil tanggal dan waktu sistem saat ini secara real-time
--              : DATEDIFF() = Menghitung selisih atau jarak antara dua waktu/tanggal berdasarkan satuan tertentu (hari, bulan, tahun)
...
-- Memanggil VIEW:
SELECT * FROM vw_LaporanStatusPendadaranLengkap ORDER BY Tanggal_Ujian DESC;
GO
...
FROM dbo.Mahasiswa AS m
ORDER BY TotalBimbinganMahasiswa DESC;
GO
...
BEGIN                                  
    ...
    IF EXISTS (                                    -- Perintah untuk mengecek apakah query di dalam kurung menghasilkan baris data
        SELECT 1 
...
-- 1. UJI COBA GAGAL (Sengaja memasukkan tanggal ujian hari Minggu: '2026-05-17')
-- Trigger akan otomatis mencegat dan membatalkan (ROLLBACK) penyimpanan data ini.
INSERT INTO dbo.Pengajuan_Pendadaran 
(
    NIM, NIP_Staff, NIDN_Penguji1, NIDN_Penguji2, 
    IPK, SKS_Lulus, Tanggal_Pengajuan, Tanggal_Ujian, Ruangan, Status, Nilai, Catatan
)
VALUES 
(
    '20214350005', '199001012020011001', '0511019001', '0512029102', 
    3.50, 135, '2026-05-01', '2026-05-17', 'Lab Komputer 1', 'Menunggu', NULL, 'Uji Coba Hari Minggu (Ditolak Trigger)'
);
GO

-- 2. UJI COBA BERHASIL (Memasukkan tanggal ujian hari Senin / hari kerja: '2026-05-18')
-- Karena bukan hari Minggu, validasi trigger lolos dan data berhasil masuk ke database.
INSERT INTO dbo.Pengajuan_Pendadaran 
(
    NIM, NIP_Staff, NIDN_Penguji1, NIDN_Penguji2, 
    IPK, SKS_Lulus, Tanggal_Pengajuan, Tanggal_Ujian, Ruangan, Status, Nilai, Catatan
...
VALUES 
(
    '20214350016', '199102022020012002', '0512029102', '0515059405', 
    2.85, 131, '2026-05-01', '2026-05-18', 'Lab Komputer 1', 'Menunggu', NULL, 'Uji Coba Hari Senin (Lolos)'
);
GO

-- 3. MELIHAT HASILNYA DI DATABASE
SELECT ID_Pengajuan, NIM, Tanggal_Ujian, Ruangan, Status, Catatan 
FROM dbo.Pengajuan_Pendadaran 
...
EXEC dbo.usp_TambahPengajuanPendadaran 
    @NIM = '20214350019',               -- Mengisi parameter NIM mahasiswa (Vino G. Bastian)
...
-- Membuat atau memperbarui Stored Procedure di dalam database
CREATE OR ALTER PROCEDURE dbo.usp_TambahPengajuanPendadaranTugas9
...
-- Perintah untuk membuat stored procedure baru atau memperbarui secara otomatis jika sudah ada di dalam database
CREATE OR ALTER PROCEDURE dbo.usp_TambahPengajuanPendadaranTugas10
    @NIM CHAR(11),                        -- Parameter input untuk menerima data NIM mahasiswa
    @Tanggal_Ujian DATE,                  -- Parameter input untuk menerima data tanggal rencana ujian pendadaran
    @Ruangan VARCHAR(50),                 -- Parameter input untuk menerima data lokasi ruangan ujian
    @NIDN_Penguji1 CHAR(10),              -- Parameter input untuk menerima data NIDN dosen penguji pertama
